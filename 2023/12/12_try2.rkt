...
(define (split-at-dot current sequence)
  (local ((define (break current sequence)
            (cond
              [(member current ".")]
              [else (list current sequence)]
              
              
(define (all-placements result current sequence) ;; Returns a list of (result current sequence)'s
